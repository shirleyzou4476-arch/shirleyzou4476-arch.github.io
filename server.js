...
const routing={ 'SKU-ALP-01':['A-01-03','A-01-04'], 'SKU-BLU-07':['B-02-01','B-02-02'], 'SKU-CED-03':['C-01-02','C-01-03'] };
const scanEvents=[];
...
const server=http.createServer(async(req,res)=>{
  ...
  if(path==='/api/scans'&&req.method==='POST'){
    ...
    const box=boxes[String(input.boxId||'').toUpperCase()]||input;
    if(!box.boxId||!box.sku||!Number.isInteger(box.qty)||box.qty<1)return send(res,400,{error:'boxId, sku and positive integer qty are required'});
    const previous=scanEvents.find(x=>x.boxId===box.boxId);
    if(previous)return send(res,409,{error:'Duplicate scan',previous});
    const destination=(routing[box.sku]||['OVERFLOW'])[0];
    const event={boxId:box.boxId,sku:box.sku,qty:box.qty,destination,userId:input.userId||'demo-user',deviceId:input.deviceId||'demo-device',inboundId:input.inboundId||null,clientId:input.clientId||null,boxSequence:input.boxSequence||null,scannedAt:new Date().toISOString()};
    scanEvents.push(event);return send(res,201,event);
  }
...
export {server,boxes,scanEvents};
